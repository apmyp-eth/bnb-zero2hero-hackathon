const CONTRACT_ADDRESS = "0xBf077822a20dECE7A03d0489c1d601790F1152bF";
const ABI = [
  {
    "anonymous": false,
    "inputs": [
...
async function prevRound(roundId, untilRound) {
    if(roundId<0 || roundId<=untilRound) {
        ...
        document.querySelector('.price-last').innerHTML=`Last price: ${historicalPrices[0]} <small class="text-body-secondary price-last-timestamp">at ${historicalLabels[0]} UTC+0</small>`;
        return;
    }
...
async function updatePrices() {
  ...
  let untilRound = lastRoundId-currentPeriod;
